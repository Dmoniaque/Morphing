        ...
        for (int x=0; x<I_inter->largeur; x++){
            ...
            for (int t=0; t<I_inter ->nb_triangle; t++){
                ...
                if(dedans_triangle(P, I_inter->point[iA], I_inter->point[iB], I_inter->point[iC])){
                    ...
                    COULEUR cdepart= I_depart.pixels[y_depart][x_depart];
                    COULEUR carrive= I_arrive.pixels[y_arrive][x_arrive];

                    int rd=(cdepart >>16) &0xFF;
                    ...
                    int rf=(int)((1.0-alpha)*rd +alpha*ra);
                    int gf=(int)((1.0-alpha)*gd +alpha*ga);
                    int bf=(int)((1.0-alpha)*bd +alpha*ba);

                    I_inter->pixels[y][x]= couleur_RGB_stock(rf, gf, bf);
                    break;
                }
            }
        }
